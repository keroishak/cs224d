...
class RNN_Model():
    ...
    def inference(self, tree, predict_only_root=False):
        ...
        if predict_only_root:
            ...
            node_tensors=tf.reshape(node_tensors,[1,self.config.embed_size])
        else:
            node_tensors = [tensor for node, tensor in node_tensors.iteritems() if node.label!=2]
            node_tensors = tf.stack(node_tensors)
            #print tf.shape(node_tensors).eval()
    ...
    def add_model_vars(self):
        ...
        '''
        self.embedding = tf.get_variable('embedding', shape=(len(self.vocab), self.config.embed_size))
        ...
        with tf.variable_scope('Projection'):
            ...
            bs = tf.get_variable('b1', shape=(1, self.config.label_size))
            ### END YOUR CODE
    ...
    def add_model(self, node):
        ...
        if node.isLeaf:
            ### YOUR CODE HERE
            id=self.vocab.encode(node.word)
            curr_node_tensor=tf.nn.embedding_lookup(self.embedding,id)
            ### END YOUR CODE
        else:
            node_tensors.update(self.add_model(node.left))
            node_tensors.update(self.add_model(node.right))
            ### YOUR CODE HERE
            #print tf.shape(node.left)
            leftRightNode = tf.concat([node_tensors[node.left], node_tensors[node.right]],0)
            leftRightNode=tf.reshape(leftRightNode,[1,2*self.config.embed_size])
            curr_node_tensor = tf.nn.relu(tf.matmul(leftRightNode, w1) + b1)
            ### END YOUR CODE
        node_tensors[node] = tf.reshape(curr_node_tensor,[-1])
        return node_tensors
    ...
    def loss(self, logits, labels):
        ...
        loss=tf.reduce_sum(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits,labels=labels))+self.config.l2*l2_loss
        # END YOUR CODE
        return loss
    ...
    def train(self, verbose=True):
        ...
        for epoch in xrange(self.config.max_epochs):
            ...
            if val_loss < best_val_loss:
                # shutil.copy2('./weights/%s.temp'%self.config.model_name, './weights/%s'%self.config.model_name)
