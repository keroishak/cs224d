...
class NERModel(LanguageModel):
  ...
  def add_placeholders(self):
    ...
    """
    ### YOUR CODE HERE
    self.input_placeholder=tf.placeholder(shape=[None,self.config.window_size],dtype=tf.int32)
    self.labels_placeholder=tf.placeholder(shape=[None,self.config.label_size],dtype=tf.int32)
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ### YOUR CODE HERE
      params=tf.constant(self.wv,dtype=tf.float32)
      embedding=tf.nn.embedding_lookup(params,ids=self.input_placeholder)
  ...
  def add_model(self, window):
    ...
    with tf.variable_scope('hidden_layer') as hidden_layer:
      ...
      b2 = tf.get_variable('b2', [self.config.label_size],
                             initializer=xavier_weight_init())
    regLoss=tf.reduce_sum(tf.square(w))+tf.reduce_sum(tf.square(b1))+tf.reduce_sum(tf.square(u))+ \
            tf.reduce_sum(tf.square(b2))
    tf.add_to_collection('regLoss',regLoss)
    #print window.get_shape()
    hidden=tf.nn.tanh(tf.matmul(window,w)+b1)
    hidden=tf.nn.dropout(hidden,tf.to_float(self.dropout_placeholder))
    probs=tf.nn.softmax(tf.matmul(hidden,u)+b2)
    output=tf.nn.dropout(probs,tf.to_float(self.dropout_placeholder))
    ### END YOUR CODE
    return output
  ...
  def add_loss_op(self, pred):
    ...
    """
    ### YOUR CODE HERE
    regLoss=self.config.l2*tf.get_collection('regLoss')[0]
    loss=tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=self.labels_placeholder,logits=pred))+regLoss
    ### END YOUR CODE
    return loss
