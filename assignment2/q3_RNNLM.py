...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    L = tf.get_variable("embeddings", dtype=tf.float32, shape=(len(self.vocab), self.config.embed_size))
    with tf.device('/cpu:0'):
        ### YOUR CODE HERE

      embeddings=tf.nn.embedding_lookup(params=L,ids=self.input_placeholder)
      splittedList= tf.split(embeddings,self.config.num_steps,1)
      inputs=[tf.squeeze(item) for item in splittedList]
      ### END YOUR CODE
    return inputs
  ...
  def add_projection(self, rnn_outputs):
    ...
    u=tf.get_variable(name="Uweights",shape=[self.config.hidden_size,length])
    b=tf.get_variable(name="biases",shape=[length])
    outputs=[]
    for item in rnn_outputs:
      mult=tf.matmul(item,u)+b
      #res=tf.nn.dropout(mult,self.config.dropout)
      outputs.append(mult)
    ### END YOUR CODE
    return outputs
  ...
  def add_loss_op(self, output):
    ...
    loss=sequence_loss(logits=output,targets=tf.to_int32(self.labels_placeholder),weights=weights)
  ...
  def __init__(self, config):
    ...
    output = tf.reshape(tf.concat(axis= 1,values= self.outputs), [-1, len(self.vocab)])
  ...
  def add_model(self, inputs):
    ...
    with tf.variable_scope('rnn') as scope:
        self.initial_state=tf.zeros(name="initialState", shape=(self.config.batch_size,self.config.hidden_size))
        h=tf.get_variable("HWeight",shape=(self.config.hidden_size,self.config.hidden_size))
        i = tf.get_variable("IWeight", shape=(self.config.embed_size, self.config.hidden_size))
        b1=tf.get_variable("Bias",shape=(self.config.hidden_size))
    rnn_outputs = []
    state = self.initial_state
    for input in inputs:
      x = tf.nn.dropout(input, self.dropout_placeholder)
      res = tf.sigmoid(tf.matmul(state, h) + tf.matmul(x, i) + b1)
      state = res
      rnn_outputs.append(tf.nn.dropout(res, self.dropout_placeholder))
